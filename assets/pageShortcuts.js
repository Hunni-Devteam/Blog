export default [
  {
    created_at : '2019-05-20 03:30:00',
    title: 'Disqus 연동 테스트',
    description: 'Disqus 연동 테스트',
    tags: ['development', 'disqus', 'blog-development'],
    src: '/article/development/disqus-comment-test',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-05-23 21:30:00',
    title: 'Laravel 쿼리 빌더 사용으로 인한 MSSQL 성능 이슈',
    description: '컬럼 타입 강제 캐스팅으로 인해 인덱스를 타지 않았다.',
    tags: ['draft', 'development', 'web-backend', 'laravel', 'perfomance', 'issue'],
    src: '/article/development/laravel-query-builder-causes-mssql-perfomance-issue',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-05-24 18:00:00',
    title: 'Cordova-Based SPA에 핫 리로딩 적용하기',
    description: 'config.xml - content 태그 값 변경으로 손쉽게 적용할 수 있어요.',
    tags: ['draft', 'development',  'web-frontend', 'cordova', 'vue', 'tip'],
    src: '/article/development/usage-of-cordova-remote-page-content',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-05-24 18:00:00',
    title: 'Cordova + Vue.js 연동하기',
    description: 'Vue CLI 3 너무 편해요!',
    tags: ['draft', 'development',  'web-frontend', 'cordova', 'vue', 'tip'],
    src: '/article/development/use-cordova-with-vue-cli-3',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-07-07 15:40:00',
    title: 'Vuex, 커링, 클로저.',
    description: '알게 모르게 써오던 개념 정리',
    tags: ['development', 'web-frontend', 'vue', 'tip', 'ES6', 'functional-programming'],
    src: '/article/development/vuex-currying-closure',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-07-15 08:54:00',
    title: 'cordova-plugin-firebase 오류 기록 - Part 1',
    description: '플랫폼 의존적인 플러그인 오류 분석(실패)..',
    tags: ['development', 'cordova', 'android', 'issue'],
    src: '/article/development/cordova-plugin-firebase-issue-part1',
    image: '/disqus-cover.png'
  },
  {
    created_at : '2019-07-15 08:57:00',
    title: 'cordova-plugin-firebase 오류 기록 - Part 2',
    description: '오류 내용 분석과 조치 방법 정리',
    tags: ['development', 'cordova', 'android', 'issue'],
    src: '/article/development/cordova-plugin-firebase-issue-part2',
    image: '/disqus-cover.png'
  }
]